...
export default function StoneComponent({ item, className, displayMode }: {
    item: Item,
    className?: string
        ...
        <div className="flex max-md:flex-col gap-2 items-start md:items-center flex-1 max-md:my-2">
            ...
            <div className="flex flex-col md:gap-2 flex-1">
                ...
                <div className="flex flex-wrap gap-2 items-baseline md:justify-end">
                    <Badge className={`bg-green-100 text-green-800 transition duration-300 ${displayMode === DisplayMode.BUY ? 'shadow-lg shadow-white/20' : ''}`}>
                        Buy : <span className="font-medium">{nFormatter(item.buyPriceToPerfect, 2)}</span>
                    </Badge>
                    <Badge className={`bg-red-100 text-red-800 transition duration-300 ${displayMode === DisplayMode.SELL ? 'shadow-lg shadow-white/20' : ''}`}>
